...
N = [0, 1, 2, 3, 0, 4, 5, 0]  # пример списка значений n

# result = [res for res in N if ThreadPoolExecutor().submit(T, res).result() > 0]
result = [f.result() for f in [ThreadPoolExecutor().submit(T, n) for n in N] if f.result() > 0]
print (result)